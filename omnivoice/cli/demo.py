...
def translate_text(text: str, target_lang: str) -> str:
    ...
    if not text or target_lang == "Auto":
        return text
    try:
        # Resolve display name to ISO code
        lang_code = _resolve_language(target_lang.split(" / ")[0]) if " / " in target_lang else _resolve_language(target_lang)
        if not lang_code:
            return text
        
        url = f"https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl={lang_code}&q={text}"
        res = requests.get(url, timeout=5).json()
        return " ".join([segment[0] for segment in res[0]])
...
"""
        ...
        with gr.TabItem("Voice Clone"):
            with gr.Row():
                with gr.Column(scale=1):
                    ...
                    gr.Markdown(
                        "<<spanspan style='font-size:0.85em;color:#888;'>"
        ...
        with gr.TabItem("Voice-to-Voice Translation"):
            with gr.Row():
                with gr.Column(scale=1):
                    v2v_audio_in = gr.Audio(
                        label="Source Audio / 源音频",
                        type="filepath",
                        elem_classes="compact-audio",
                    )
                    v2v_lang = _lang_dropdown("Target Language / 目标语种")
                    v2v_trans_text = gr.Textbox(
                        label="Translated Text / 翻译文本",
                        lines=3,
                        interactive=False,
                    )
                    with gr.Accordion("Generation Settings (optional)", open=False):
                    ...
                    v2v_btn = gr.Button("Translate & Convert / 翻译并转换", variant="primary")
                with gr.Column(scale=1):
                    v2v_audio_out = gr.Audio(
                        label="Translated Voice / 翻译后的声音",
                        type="numpy",
                    )
                    v2v_status = gr.Textbox(label="Status / 状态", lines=2)

def _v2v_fn(ref_audio, target_lang, ns, gs, dn, sp, du, pp, po):
    if not ref_audio:
        return None, "", "Please upload audio."
    
    try:
        # 1. ASR Transcription
        original_text = model.transcribe(ref_audio)
        
        # 2. Real Translation
        # Resolve display name to ISO code
        lang_code = _resolve_language(target_lang.split(" / ")[0]) if " / " in target_lang else _resolve_language(target_lang)
        
        # Use the integrated translator utility
        from omnivoice.utils.translator import translator
        translated_text = translator.translate_text(original_text, target_lang=lang_code)
        
        # 3. Timeline Preservation (Original Duration)
        wav, sr = torchaudio.load(ref_audio)
        original_duration = wav.shape[-1] / sr
        
        # 4. Voice Conversion (Clone Original Voice with 100% matching)
        audio, status = _gen(
            translated_text,
            target_lang,
            ref_audio,
        ...
        return audio, translated_text, status
    except Exception as e:
        return None, "", f"Error: {str(e)}"
            ...
            v2v_btn.click(
                ...
                outputs=[v2v_audio_out, v2v_trans_text, v2v_status],
            )
