...
class OmniVoice(PreTrainedModel):
    ...
    def translate_and_generate(
        ...
        ref_audio: Union[str, list[str], tuple[torch.Tensor, int], list[tuple[torch.Tensor, int]], None] = None,
        ref_text: Union[str, list[str], None] = None,
        **kwargs
    ) -> list[torch.Tensor]:
        """
        Deeply translates text and generates audio while preserving the original voice, 
        emotion, and timing. Supports batch processing for multiple speakers.
        """
        ...
        target_langs = self._ensure_list(target_lang, batch_size) if target_lang else [None] * batch_size
        
        translated_texts = []
        for i in range(batch_size):
            t_text = text_list[i]
            t_lang = target_langs[i]
            if t_lang and t_lang != "Auto":
                # Resolve language display name to ISO code using the helper function defined in the module
                lang_code = _resolve_language(t_lang)
                translated_texts.append(translator.translate_text(t_text, target_lang=lang_code))
            else:
                translated_texts.append(t_text)
        
        return self.generate(
            text=translated_texts,
            language=target_langs,
            ref_audio=ref_audio,
            ref_text=ref_text,
            **kwargs
        )
